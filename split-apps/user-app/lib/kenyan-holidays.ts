 ...
 * Uses the algorithm for calculating Easter Sunday
 */
...
/**
 * Get Eid al-Fitr date (approximate - based on lunar calendar)
 * This is an approximation; for accurate dates, use Islamic calendar API
 */
export function getEidAlFitrDate(year: number): Date {
  // Approximate dates based on lunar calendar
  // These are estimates and may vary by 1-2 days
  const approxDates: Record<number, Date> = {
    2024: new Date(2024, 3, 10), // April 10, 2024
    2025: new Date(2025, 2, 30), // March 30, 2025
    2026: new Date(2026, 2, 20), // March 20, 2026
    2027: new Date(2027, 2, 9),  // March 9, 2027
  };
...
/**
 * Get Eid al-Adha date (approximate - based on lunar calendar)
 * This is an approximation; for accurate dates, use Islamic calendar API
 */
export function getEidAlAdhaDate(year: number): Date {
  // Approximate dates based on lunar calendar
  const approxDates: Record<number, Date> = {
    2024: new Date(2024, 5, 17), // June 17, 2024
    2025: new Date(2025, 5, 6),  // June 6, 2025
    2026: new Date(2026, 4, 26), // May 26, 2026
...
export function isHoliday(date: Date): {
  isHoliday: boolean;
  holiday?: typeof KENYAN_HOLIDAYS[0];
} {
 ...
 */
export function getUpHolidays(fromDate: Date = new Date(), count: number = 5): Array<{
  name: string;
  date: Date;
  daysUntil: number;
  type: string;
...
/**
 * Get event type suggestions based on date/holiday
 */
export function getEventSuggestions(date: Date): string[] {
  const suggestions: string[] = [];
  const holidayInfo = isHoliday(date);
  
  if (holidayInfo.isHoliday) {
    suggestions.push(`Holiday Event: ${holidayInfo.holiday?.name}`);
  }
  ...
  if (dayOfWeek === 0 || dayOfWeek === 6) {
    suggestions.push("Weekend Shoot", "Family Portrait", "Couple Session");
  }
  
  // Month-based suggestions
  const month = date.getMonth();
  if (month === 11) { // December
    suggestions.push("Christmas Photos", "New Year Celebration", "Corporate Party");
  } else if (month === 0) { // January
    suggestions.push("New Year Session", "Back to School");
  } else if (month === 5 || month === 6) { // June-July
    suggestions.push("Graduation Photos", "University Ceremony");
  } else if (month === 9 || month === 10) { // October-November
    suggestions.push("Graduation Photos", "Corporate Events");
  }
  
  // Day-based suggestions
  if (dayOfMonth === 14) {
    suggestions.push("Valentine's Day Shoot");
  } else if (dayOfMonth === 25) {
    suggestions.push("Christmas Photos");
  }
