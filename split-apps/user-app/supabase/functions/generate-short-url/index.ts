...
serve(async (req) => {
  ...
  try {
    ...
    // Construct short URL — env var > platform_settings > hardcoded fallback
    let baseUrl = Deno.env.get("SHORT_URL_BASE");
    if (!baseUrl) {
      const { data: setting } = await supabase
        .from("platform_settings")
        .select("value")
        .eq("key", "short_url_base")
        .maybeSingle();
      baseUrl = setting?.value || "https://epx.vc";
    }
