...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "get_code") {
      ...
      if (existing) {
        // Ensure referral_token exists
        let token = existing.referral_token;
        if (!token) {
      ...
      const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-";
      let token = Array.from(crypto.getRandomValues(new Uint8Array(12)))
        .map(b => chars[b % 64])
        .join("");
      ...
      // Mark a click as converted if referral_token matches
      if (result?.success && referral_code) {
        await supabase.rpc("mark_referral_converted", {
          p_referral_token: referral_code,
          p_user_id: user.id,
        });
      }
      ...
      // Generate share link using referral_token
      ...
      // Get or create the referral token for this user
      ...
      let token = referral?.referral_token;
      if (!token) {
        const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-";
        token = Array.from(crypto.getRandomValues(new Uint8Array(12)))
          .map(b => chars[b % 64])
