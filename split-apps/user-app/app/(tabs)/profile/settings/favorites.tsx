...
export default function Favorites() {
  ...
  const downloadAll = async () => {
    ...
    // Group favorites by gallery_id and navigate to the first gallery
    const galleryIds = [...new Set(favorites.map(f => f.gallery_id).filter(Boolean))];
        ...
        { 
          ...
          onPress: () => {
            ...
            if (galleryIds.length === 1) {
              router.push(`/(tabs)/gallery?galleryId=${galleryIds[0]}`);
            } else if (galleryIds.length > 1) {
              router.push(`/(tabs)/gallery`);
            } else {
              Alert.alert('Error', 'No gallery information found for favorites');
            }
          }
        }
      ]
