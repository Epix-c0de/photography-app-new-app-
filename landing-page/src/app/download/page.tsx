...
import { Download, Smartphone, ArrowLeft, CheckCircle, Loader2, AlertCircle } from 'lucide-react';
...
export default function DownloadPage() {
  ...
  useEffect(() => {
    fetch('/api/apk/download?type=client&action=metadata')
      .then(r => r.json())
      .then(data => {
        if (data.error) {
          setError(data.error);
  ...
  const handleDownload = async () => {
    ...
    try {
      if (clientApk.chunked && clientApk.chunk_urls) {
        // Chunked: fetch all chunks in browser and save as single blob
        const chunks: Blob[] = [];
        for (let i = 0; i < clientApk.chunk_urls.length; i++) {
          const res = await fetch(clientApk.chunk_urls[i]);
        ...
        // Single chunk: direct download from signed URL
        const a = document.createElement('a');
                ...
                <>
                  {clientApk && (
                    <div className="flex items-center gap-2 mb-4 text-xs text-white/40">
                      ...
                      <span>•</span>
                      <span>{formatSize(clientApk.file_size)}</span>
                      {clientApk.chunked && (
                        <>
                          <span>•</span>
                          <span>{clientApk.chunk_count} parts</span>
                        </>
                      )}
                    </div>
                  )}
