...
import { Smartphone, ArrowRight, Film } from 'lucide-react';
...
export default function BtsSharePage() {
    const params = useParams();
    const id = params?.id as string;
    const [bts, setBts] = useState<any>(null);
    ...
    useEffect(() => {
        if (!id) return;
        fetch(`${SUPABASE_URL}/rest/v1/bts_posts?id=eq.${id}&select=id,title,content,created_at,image_url,video_url`, {
            headers: {
                apikey: SUPABASE_KEY,
            ...
            .then(r => r.json())
            .then(data => {
                if (data?.length > 0) setBts(data[0]);
                else setError(true);
            })
            .catch(() => setError(true))
    ...
    const deepLink = `epix-visuals://bts?id=${id}`;
                    ...
                    <div className="mb-8 text-center">
                        ...
                        {loading ? (
                            <div className="space-y-2 animate-pulse">
                                <div className="h-7 bg-white/10 rounded w-56 mx-auto" />
                                <div className="h-4 bg-white/10 rounded w-64 mx-auto" />
                            </div>
                        ) : (
                            <>
                                <h2 className="text-2xl font-bold text-white mb-2">
                                    {bts?.title || 'Behind the Scenes'}
                                </h2>
                                {bts?.description && (
                                    <p className="text-white/60 text-sm line-clamp-3">{bts.description}</p>
                                )}
                                {!error && !bts && (
                                    <p className="text-white/60 text-sm">Content not found or no longer available.</p>
                                )}
                                {error && (
                                    <p className="text-red-400 text-sm">Unable to load content.</p>
                                )}
                            </>
                        )}
                    </div>

                    {!loading && bts?.created_at && (
                        <div className="mb-6 p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                            <p className="text-white/50 text-xs uppercase tracking-wider mb-1">Shared</p>
                            <p className="text-white font-medium">
                                {new Date(bts.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                            </p>
                        </div>
                    )}
                    {loading && (
                    ...
                    {isMobile && !triedDeepLink && (
                        <>
                            ...
                            <p className="text-center text-white/40 text-xs mb-6">
                                Open the app to watch behind the scenes content
                            </p>
                        </>
                    )}
                    ...
                    {isMobile && triedDeepLink && (
                        <>
                            <div className="mb-4 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-center">
                                ...
                                <p className="text-white/50 text-xs mt-1">Download it to watch BTS content</p>
                            </div>
                            ...
                            <p className="text-center text-white/40 text-xs mt-4">
                                After installing, open the link again to view BTS content
                            </p>
                        </>
                    )}
