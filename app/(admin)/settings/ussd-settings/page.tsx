...
import { 
  Phone, Settings, Save, TestTube, CheckCircle, XCircle, 
...
export default function USSDSettingsPage() {
  ...
  const testUSSD = async () => {
    ...
    try {
      const testCode = settings.shortCode.replace(/\*/g, '').replace(/#/g, '');
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '400px', color: 'rgba(255,255,255,0.5)' }}>
        <Loader2 size={24} style={{ animation: 'spin 1s linear infinite' }} /> Loading USSD settings...
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: 16, fontWeight: 700, color: '#D4AF37', marginBottom: 12 }}>Need Help?</h3>
        <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.7)', marginBottom: 16 }}>
          Contact {providerInfo[settings.provider].name} to get your USSD short code:
        </p>
        <div style={{ display: 'flex', gap: 12 }}>
          <a
            href={`tel:${providerInfo[settings.provider].contact?.replace(/[^\d+]/g, '')}`}
            style={{
              padding: '10px 20px',
              borderRadius: 10,
              border: '1px solid rgba(212,175,55,0.3)',
          ...
          >
            <Phone size={14} /> Call {providerInfo[settings.provider].contact}
          </a>
          {providerInfo[settings.provider].website && (
            <a
              href={providerInfo[settings.provider].website}
